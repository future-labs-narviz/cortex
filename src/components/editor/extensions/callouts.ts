...
// ---------------------------------------------------------------------------
// Icon widget — renders the SVG icon inline at the start of the header line
// ---------------------------------------------------------------------------
...
class CalloutIconWidget extends WidgetType {
  ...
  toDOM() {
    ...
    span.style.cssText =
      "display:inline-flex;align-items:center;margin-right:6px;vertical-align:middle;opacity:0.85;";
    return span;
  }
...
function buildCalloutDecorations(view: EditorView): DecorationSet {
  ...
  while (i <= doc.lines) {
    ...
    // Insert icon widget at start of line content (after the "> [!TYPE] " prefix)
    const prefixMatch = line.text.match(/^>\s*\[!\w+\]\s*/);
    if (prefixMatch) {
      const iconPos = line.from + prefixMatch[0].length;
      decorations.push(
        Decoration.widget({
          widget: new CalloutIconWidget(config.icon, config.label),
          side: -1,
        }).range(iconPos),
      );
    }
